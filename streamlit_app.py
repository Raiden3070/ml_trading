...
from ml_trading import MLTrading
from set_indicator_trading import trades_to_orders
...
def compute_latest_signal(sl: MLTrading, symbol: str, prices: pd.Series) -> tuple[int, pd.Timestamp]:
...
def main():
    ...
    with st.spinner("Training ML model…"):
        sl = MLTrading(impact=impact, commission=commission)
        sl.append_training_data(symbol=symbol, sd=tr_sd, ed=tr_ed, sv=100000, price_series=prices_train)
